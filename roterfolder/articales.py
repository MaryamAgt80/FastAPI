...
from auth.auth2 import schema_auch, get_current_user

router = APIRouter(prefix='/articales', tags=['articales'])
from roterfolder.userfile import usermodel
...
def createarticle(request: articalemodel, DB=Depends(get_db), curret_user: usermodel = Depends(get_current_user)):
